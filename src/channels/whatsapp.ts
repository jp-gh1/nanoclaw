      ...
      sock.ev.on('messages.upsert', async ({ messages }) => {
        for (const msg of messages) {
          try {
            ...
            // Detect if bot was @mentioned in a group via JID list or name
            const mentionedJids: string[] = normalized.extendedTextMessage?.contextInfo?.mentionedJid ?? [];
            const isBotMentionedInGroup =
              isGroup &&
              (mentionedJids.some(
                (jid) => (botPhoneJid && jid === botPhoneJid) || (botLidUser && jid === `${botLidUser}@lid`),
              ) ||
            ...
            const inbound: InboundMessage = {
              ...
              isMention: !isGroup ? true : isBotMentionedInGroup ? true : undefined,
              isGroup,
              content: {
                text: content,
                sender,
